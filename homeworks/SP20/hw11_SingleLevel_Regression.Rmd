...
a) Fit a regression of child test scores on mother's age, display the data and fitted model, check assumptions, and interpret the slope coefficient. When do you recommend mothers should give birth? What are you assuming in making thees recommendations?
...
c) Now create an indicator variable `hs` reflecting whether the mother has completed high school or not, assuming that `educ_cat` = 1 indicates *did not complete High School*, so that `hs` = 1 if `educ_cat` > 1. Consider interactions between the high school completion and mother's age in family. Also, create a plot that shows the separate regression lines for each high school completion status group.
...
a) In R, check the dataset and clean any unusually coded data (hint: see `23_SingleLevel_Reg2_SimModelsInf_20apr2020.Rmd` for code related to this).
...
c) Fit some regression models with the goal of predicting earnings from some combination of sex, height, and year born. Be sure to try various transformations and interactions that make sense. Choose your preferred model and justify.
