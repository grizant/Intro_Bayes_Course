...
## treament group
cd4 <- subset(cd4, cd4$treatmnt == 1)
## inclusion for age range
cd4 <- subset(cd4, (baseage>1 & baseage<5))
## remove missing y
cd4 <- subset(cd4, !is.na(CD4PCT))
## summary(cd4)
cd4$y <- sqrt(cd4$CD4PCT) ## meets additivity assumption better
cd4$time <- cd4$visage - cd4$baseage
## set up new patient id numbers from 1 to J "groups"
cd4$pid <- factor( as.numeric( factor(cd4$newpid) ) )
```

a) Graph the outcome variable `y` (CD4 percentage, on the square root scale) for each child as a function of `time`). Since there are over 80 children, panels may be too many plots. Try a "spaghetti plot" instead. See this [post](https://stats.idre.ucla.edu/r/faq/how-can-i-visualize-longitudinal-data-in-ggplot2/) for guidance.

b) Each child's data has a time course that can be summarized by a linear fit. Estimate these lines and plot them for all children. See this [post](https://stackoverflow.com/questions/31903606/ggplot2-plotting-regression-lines-with-different-intercepts-but-with-same-slope) with plotting the fits.
...
b) Extend the model in a) to include child-level predictors (that is, group-level predictors) for time and age at baseline. Fit using `lmer()` and interpret the coefficients for time, and age at baseline.

c) **BONUS 5 pts**: Investigate the change in partial pooling from a) to b) both graphically and numerically.
