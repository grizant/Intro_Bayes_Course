---
title: "Ch.13 Diff between Means"
author: "AG Schissler"
date: "3/25/2020"
...
1. Think carefully about how many times a workday you check email in a 24-hour period.
...
2. Construct the posterior distribution for each group, assumming independent random samples from two normal distributions and known standard deviation of 3.
...
Any brave volunteer willing to share their screen?

#### 95% credible interval for the mean differece with unknown but equal variances

## Case 2. 13.3. Inferences with unequal variances

Discuss using textbook (nonadditive).

## Now let's do the same analysis with known, unequal variances.

- Then compute the posterior parameters under independent sampling and priors. The updating rules are **exactly** the same!
...
### Now assume the variances are unequal & unknown (most general but worse precision)
...
## III. 13.5 Finally let's collect data and think about a paired designed (dependent samples) 
...
c( post_param$m_prime - ( t_star * post_param$s_prime ) ,
post_param$m_prime + (  t_star * post_param$s_prime ) )
## easy way, just compute the classical frequentist version
t.test(x = diff_in_email, conf.level = 0.95)
```
