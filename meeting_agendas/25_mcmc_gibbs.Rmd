...
<bring to class: laptop, BIDA textbook>
...
Suppose we have data $y$ and now we'd like to sample from the posterior. We'll use simulated data to be sure that our Gibbs sampler is doing its job.
...
Now we'll visualize the **survival function**, $S(t) = Pr( T > t ) = 1 - F(t) =  e^{-\lambda {t}^\alpha}$ using the posterior samples. 
