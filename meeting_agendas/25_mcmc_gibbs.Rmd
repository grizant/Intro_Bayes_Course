...
# What is Gibbs sampling (MCMC) and how do we use it to do Bayesian inference?
...
for (i in 2:reps) {
    ...
    mu_i <- rnorm(n = 1, mean = mu_hat, sd = sqrt( n_times_tau + b ) )
    ## then sample tau
    shape <- c + (n/2)
...
plot( x = as.vector(chain1[,1]), y = as.vector(chain1[,2]), xlab = expression(mu), ylab = expression(tau) )
```
