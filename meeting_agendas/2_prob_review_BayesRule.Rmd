...
<Draw probablity tree on the ipad>

# 1. Administration items

## learning outcomes omitted in meeting 1
...
https://unr.canvaslms.com/courses/65495/assignments/739522

# 2. Probability review

## Probability terms

Please define and notate (when appropriate) without referring to the text or Internet. 
...
3. If $A$ and $B$ are _mutually exclusive_ events, then $P(A \cup B) = P(A) + P(B)$. (probability is _additive_ over disjoint events).e
## Joint probability example revisited 

## Now let's discuss a realistic and (perhaps) surprising twist to the birth calculation:

```{r, babies}
prob_female <- 0.487 ## actual rate of female human babies
prob_male <- 1 - prob_female
## probability four male births
prob_male^4
1/16 ## compare to crude estimate.
## bgbg
...
A restatement of the conditional probability:
...
For two events $A,B$ we can restate conditional probability in a powerful new form called Bayes' Theorem or Bayes' rule:

$P( B | A) = \frac{P( A \cap B) }{P(A)} = \frac{ P(A|B) \times P(B) }{P(A)}$.

For a finite collection of events we can also state Bayes' rule as:
