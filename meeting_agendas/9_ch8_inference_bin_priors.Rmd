...
author: "AG Schissler"
date: "24 Feb 2021 (*updated: `r Sys.Date()`)*"
output:
...
- Discuss Ch. 8 with extended examples
- Preview Ch. 8 HW.
...
## Evaluation of your prior

When selecting a prior, graph, compute moments (mean and standard deviation), and the *equivalent sample size* before combining with the data to make sure that you have properly specified your beliefs.
...
```{r ex81a, message=F}
## Anna's prior
Bolstad::binobp(x = 26, n = 100, a = 4.8, b = 19.2, quiet=T)
```
...
```{r ex81b, message=F}
## Bart's prior
Bolstad::binobp(x = 26, n = 100, a = 1, b = 1, quiet=T)
```
...
```{r ex81c, message=F}
## Chris's prior
Bolstad::binogcp(x = 26, n = 100, "user", pi = pi, pi.prior = pi.prior, quiet=T)
```
