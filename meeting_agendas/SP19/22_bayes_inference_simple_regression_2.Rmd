...
(y_bar <- mean(reg_data$MidtermThree))
(xy_bar <- mean(reg_data$MidtermOne * reg_data$MidtermThree))
(x2_bar <- mean(reg_data$MidtermOne^2))
(x_bar2 <- mean(reg_data$MidtermOne)^2)
```
### Please compute the least squares estimates of $\alpha_0, \beta$ and $\alpha_{\overline{x}}$.
...
my_ls_soln <- compute_simple_ls(x = reg_data$MidtermOne, y = reg_data$MidtermThree)

## x <- reg_data$MidtermOne
## y <- reg_data$MidtermThree
...
compute_sigma_hat(x = reg_data$MidtermOne, y = reg_data$MidtermThree)

summary(lm(MidtermThree ~ MidtermOne, data = reg_data))
...
bayes.lin.reg(y = reg_data$MidtermThree, x = reg_data$MidtermOne, slope.prior = "flat", intcpt.prior = "flat", plot.data = TRUE)
...
bayes.lin.reg(y = reg_data$MidtermThree, x = reg_data$MidtermOne, slope.prior = "normal", intcpt.prior = "normal", mb0=mb0, sb0=sb0, ma0=ma0, sa0=sa0, plot.data = TRUE)
