...
## How to do Bayesian statistics when you can't easily derive or numerically approximate the posterior?
...
Discuss Example 20.1. Similar to HW 10 #1.

#### Starting Unit 4: Computational Bayesian Methods and Advanced Regression

I'll make lecture notes Bolstad & Curran and  

[Data Analysis Using Regression and Multilevel/Hierarchical Models (Gelman and Hill 2007)](https://www.amazon.com/Analysis-Regression-Multilevel-Hierarchical-Models/dp/052168689X)

and  

[Statistical Computing with R (Rizzo 2006)](https://www.amazon.com/Statistical-Computing-Chapman-Hall-CRC/dp/1584885459)

possibly other sources.

## Ch. 20 intro. Bayesian Statistics: Easy in Theory, Difficult in Practice

Discussion of text material.

## Section 20.1.Direct methods for sampling the posterior
...
Classroom exercise:
A random variable $y$ is called exponential with **rate** $\lambda$ if it has cdf $F(y)= \lambda e^{- \lambda y}$ for $y \geq 0$ and $\lambda>0$. Let $y \sim exp(\lambda=2$). Use inverse probability sampling to create a random sample of size 10,000 from this exponential distribution. Reproduce Figure 20.3 to visualize your sample.

[A list of the distributions in the `stats` package](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Distributions.html)

```{r section202}
## our work goes here
my_uniform_sample <- runif(n = 1e3)
## qplot(my_uniform_sample)
my_exp_sample <- qexp(p = my_uniform_sample, rate = 2)
qplot(my_exp_sample)
...
```{r fig2010, fig.cap = "Our version of Figure 20.10"}
## Here I'll provide some code to reproduce Figure 20.10
...
- We'll see Gibbs later in this final Unit.
